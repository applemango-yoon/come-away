...
words 규칙 (아주 중요):
- 각 항목의 표제(english)는 반드시 "영어 단어(또는 짧은 구)"다. "믿는 자", "멸망하지 않고", "영생" 같은 한국어 문장 조각을 표제로 삼지 말고, 그 속의 핵심 영어 단어를 표제로 삼아라 (예: believe, perish, eternal).
- 진짜 낱말(단어) 위주로 골라라: 명사·동사·형용사 같은 어휘. 여기에 더해, 자주 쓰는 중요한 동사구(예: lay down, abide in, bring forth)를 몇 개 추가하라.
...
SCHEMA_VER = 4   # 분석 결과 형식 버전. 올리면 이전 캐시를 자동으로 무시하고 다시 분석함(→ 클로드로 갱신).
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            # 단어는 무조건 5개까지만 (AI가 더 줘도 잘라냄)
            if isinstance(data.get('words'), list):
                data['words'] = data['words'][:5]
